...
LOGOS_DIR     = os.path.join(BASE_DIR, "logos")  # Country-wise logo folder
...
# ═══════════════════════════════════════════════════════════════════════════════
#  🖼️  SMART LOGO INTEGRATION — Uses scraped logos + mapping + fallback
# ═══════════════════════════════════════════════════════════════════════════════

def load_logo_mapping() -> dict:
    """
    Load the smart logo mapping from logomaster scraper.
    Supports both v2.0 (smart) and legacy flat formats.
    Returns flat mapping: {channel_id: logo_path}
    """
    mapping = {}

    # Try v2.0 smart mapping first
    if os.path.exists(LOGO_MAP_FILE):
        try:
            with open(LOGO_MAP_FILE, encoding='utf-8') as f:
                data = json.load(f)

            # v2.0 format: {"flat": {...}, "by_country": {...}}
            if isinstance(data, dict) and 'flat' in data:
                mapping = data.get('flat', {})
                logger.info(f"🗺️  Loaded smart mapping v2.0: {len(mapping)} logos")
            # Legacy flat format: {"CNN.us": "logos/..."}
            elif isinstance(data, dict):
                mapping = data
                logger.info(f"🗺️  Loaded legacy mapping: {len(mapping)} logos")
        except Exception as e:
            logger.warning(f"⚠️  Logo mapping load failed: {e}")

    # Also scan logos/ folder directly as fallback
...
def get_channel_logo(cid: str, name: str, website: str = "", mapping: dict = None) -> str:
    """
    Smart logo resolver with priority:
    1. Scraped logo mapping (local file)
    2. iptv-org API logo
    3. Logo.dev / Clearbit / Wikimedia (online search)
    4. Default placeholder
    """
    if mapping is None:
        mapping = load_logo_mapping()

    # Priority 1: Local scraped logo
    if cid and cid in mapping:
        local_path = mapping[cid]
        # Convert to raw GitHub URL if needed
        if local_path.startswith('logos/'):
            # For GitHub Pages / raw content
            return f"https://raw.githubusercontent.com/AeonCoreX-Lab/streamx-iptv-data/main/{local_path}"
        return local_path

    # Priority 2: Try online sources (existing logic)
    online = find_logo_online(name, website)
    if online:
        return online
...
# ── Existing online logo search (unchanged) ──────────────────────────────────
...
def update_channels():
    ...
    logger.info(f"🗺️  Logo mapping: {len(logo_mapping)} logos available")
    ...
    for filename, rules in CATEGORY_RULES.items():
        ...
        for ch in to_process:
            ...
            # Always try to get best logo (not just once)
            best_logo = get_channel_logo(ch_id, ch.get('name', ''), website, logo_mapping)

            # Update if we found a better logo
            if best_logo and best_logo != current_logo and best_logo != DEFAULT_LOGO:
                ch['logoUrl'] = best_logo
                data_modified = True
                STATS["logo_fixed"] += 1
                logger.info(f"  🖼  Logo updated: {ch.get('name')}")
            elif not current_logo or current_logo == DEFAULT_LOGO:
                # No good logo found, mark as searched
        ...
        if time_remaining() > 300:
            ...
            if new_candidates:
                ...
                def _process_new(cid):
                    ...
                    # Smart logo: mapping → API → online → default
