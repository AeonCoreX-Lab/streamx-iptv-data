...
def generate_mapping(state: dict):
    ...
    for fname in os.listdir(LOGOS_DIR):
        if fname.endswith('.png'):
            ...
            mapping[cid] = f"logos/{fname}"

    with open(MAPPING_FILE, "w", encoding="utf-8") as f:
        json.dump(mapping, f, indent=2, ensure_ascii=False)

    print(f"🗺️  Mapping: {len(mapping)} entries → {MAPPING_FILE}")
...
def get_logo_dev_url(channel_name: str, website: str = None) -> str | None:
    """
    ...
    for domain in all_domains:
        ...
        try:
            api_url = f"https://img.logo.dev/{domain}?token={LOGO_DEV_KEY}&format=png&retina=true"

            r = session.head(api_url, timeout=10, allow_redirects=True)
            if r.status_code == 200:
                ct = r.headers.get('content-type', '').lower()
                if 'image' in ct:
                    return api_url

            api_url = f"https://img.logo.dev/{domain}?token={LOGO_DEV_KEY}&format=png"
            r = session.head(api_url, timeout=10, allow_redirects=True)
            if r.status_code == 200:
                ct = r.headers.get('content-type', '').lower()
                if 'image' in ct:
...
def process_channel(channel: dict, logo_dev_key: str, tvdb_token: str, iptvorg_map: dict, state: dict) -> dict:
    ...
    if logo_dev_key:
        img_url = get_logo_dev_url(name, website)
        if img_url:
